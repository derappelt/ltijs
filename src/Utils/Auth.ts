...
class Auth {
   ...
   */
  static async generatePlatformKeyPair (ENCRYPTIONKEY: string, Database: any, platformUrl: string, platformClientId: string): Promise<string> {
   ...
   * @returns {Promise}
   */
  static async validateToken (token: string, devMode: boolean, validationParameters: ValidationParameters, getPlatform: GetPlatformFn, ENCRYPTIONKEY: string, Database: any): Promise<any> {
    ...
    provAuthDebug('Response Iss claim: ' + payload.iss)
    ...
    } else if (validationParameters.iss !== payload.iss) throw new Error('ISS_CLAIM_DOES_NOT_MATCH')

    provAuthDebug('Attempting to retrieve registered platform')
    let platform: any
    if (!Array.isArray(payload.aud)) platform = await getPlatform(payload.iss as string, payload.aud as string, ENCRYPTIONKEY, Database)
    else {
      for (const aud of payload.aud) {
        platform = await getPlatform(payload.iss as string, aud, ENCRYPTIONKEY, Database)
        if (platform) break
      }
    }
    ...
    switch (authConfig.method) {
      case 'JWK_SET': {
        ...
        const res = await got.get(keysEndpoint).json<{ keys: Array<Record<string, unknown>> }>()
   ...
   */
  static async verifyToken (token: string, key: string, validationParameters: ValidationParameters, platform: any, Database: any): Promise<any> {
    provAuthDebug('Attempting to verify JWT with the given key')
    const verified = jwt.verify(token, key, {
      algorithms: [validationParameters.alg as jwt.Algorithm],
   ...
   * @param {Object} validationParameters - Validation parameters.
   */
  static async oidcValidation (token: any, platform: any, validationParameters: ValidationParameters, Database: any): Promise<boolean[]> {
    ...
    const alg = this.validateAlg(validationParameters.alg as string)
   ...
   * @param {Platform} platform - Platform object.
   */
  static async validateAud (token: any, platform: any): Promise<boolean> {
   ...
   */
  static async validateAlg (alg: string): Promise<boolean> {
   ...
   */
  static async validateMaxAge (token: any, maxAge: number | false | undefined): Promise<boolean> {
   ...
   * @param {Object} token - Id token you wish to validate.
   */
  static async validateNonce (token: any, Database: any): Promise<boolean> {
   ...
   * @param {Object} token - Id token you wish to validate.
   */
  static async claimValidation (token: any): Promise<void> {
   ...
   */
  static async getAccessToken (scopes: string, platform: any, ENCRYPTIONKEY: string, Database: any): Promise<AccessTokenResponse> {
    ...
    const access = await got.post(await platform.platformAccessTokenEndpoint(), { form: message }).json<AccessTokenResponse>()
