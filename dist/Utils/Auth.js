...
class Auth {
    ...
    static async generatePlatformKeyPair(ENCRYPTIONKEY, Database, platformUrl, platformClientId) {
        let kid = crypto_1.default.randomBytes(16).toString('hex');
    ...
    static async validateToken(token, devMode, validationParameters, getPlatform, ENCRYPTIONKEY, Database) {
        ...
        if (!Array.isArray(payload.aud))
            platform = await getPlatform(payload.iss, payload.aud, ENCRYPTIONKEY, Database);
        else {
            for (const aud of payload.aud) {
                platform = await getPlatform(payload.iss, aud, ENCRYPTIONKEY, Database);
                if (platform)
                    break;
            }
        }
        ...
        switch (authConfig.method) {
            case 'JWK_SET': {
                ...
                const res = await Http_1.default.get(keysEndpoint).json();
                ...
                const key = await rasha_1.default.export({ jwk });
    ...
    static async validateAud(token, platform) {
        ...
        if (Array.isArray(token.aud)) {
            ...
            if (token.azp && token.azp !== await platform.platformClientId())
    ...
    static async validateNonce(token, Database) {
        ...
        if (await Database.Get(false, 'nonce', { nonce: token.nonce }))
    ...
    static async claimValidation(token) {
        ...
        if (token['https://purl.imsglobal.org/spec/lti/claim/message_type'] !== 'LtiResourceLinkRequest' && token['https://purl.imsglobal.org/spec/lti/claim/message_type'] !== 'LtiDeepLinkingRequest')
        ...
        if (token['https://purl.imsglobal.org/spec/lti/claim/message_type'] === 'LtiResourceLinkRequest') {
            ...
            if (!token['https://purl.imsglobal.org/spec/lti/claim/resource_link'] || !token['https://purl.imsglobal.org/spec/lti/claim/resource_link'].id)
    ...
    static async getAccessToken(scopes, platform, ENCRYPTIONKEY, Database) {
        ...
        const token = jsonwebtoken_1.default.sign(confjwt, await platform.platformPrivateKey(), { algorithm: 'RS256', expiresIn: 60, keyid: await platform.platformKid() });
        const message = {
        ...
        const access = await Http_1.default.post(await platform.platformAccessTokenEndpoint(), { form: message }).json();
